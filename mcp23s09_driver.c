...
static ssize_t io_port_state_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int res;
...
static ssize_t io_port_state_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    io_port_state = (unsigned char)IO_MCP23S09_Get();
    return sprintf(buf, "0x%02X\n", (unsigned int)io_port_state);       //format: 0x00
}
